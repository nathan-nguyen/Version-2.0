...
public class ContactsProvider2 extends AbstractContactsProvider
	...
	try
	{
		getContext().checkPrivilegeEscalation(-1, Monitor.CONTACT_UID, 0, "android.permission.READ_CONTACTS");
	} catch (PrivilegeEscalationException pe)
	{
		Log.e("LogicDroid", "Query contacts failed because privilege escalation detected");
		if (selection == null || selection.isEmpty()) selection = "1 <> 1";
		else selection = "(" + selection + ") AND 1 <> 1";
	}
	// #########################################################################
