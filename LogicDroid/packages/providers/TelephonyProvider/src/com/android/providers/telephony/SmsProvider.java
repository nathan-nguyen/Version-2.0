...
public class SmsProvider extends ContentProvider {
	...
	//LogicDroid - Akash
	// ###################################################################
	// #                  Hook on accessing SMS Database
	// ###################################################################
	try{
		getContext().checkPrivilegeEscalation(Binder.getCallingUid(),Monitor.SMS_UID, System.currentTimeMillis(), "android.permission.READ_SMS");
	} catch (PrivilegeEscalationException pe)
	{
		Log.e("LogicDroid", "Query SMS failed because privilege escalation detected");
		if (selection == null || selection.isEmpty())
			selection = "1 <> 1";
		else
			selection = "(" + selection + ") AND 1 <> 1";
	}
  	// ####################################################################
