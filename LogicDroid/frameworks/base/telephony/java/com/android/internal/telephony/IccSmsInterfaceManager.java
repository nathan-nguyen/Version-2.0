...
public abstract class IccSmsInterfaceManager extends ISms.Stub {
	...
	try
	{
		mPhone.getContext().checkPrivilegeEscalation(Binder.getCallingUid(), Monitor.SMS_UID, System.currentTimeMillis(), "android.permission.SEND_SMS");
	}
	catch (PrivilegeEscalationException pe)
	{
		// do nothing, just log that it has been blocked
		Log.e("LogicDroid", "Sending SMS from " + scAddr + " to " + destAddr + " is blocked because Privilege Escalation is detected"); 
		return;	
	}
	// ##################################################################
