...
static void throwErrnoException(JNIEnv* env, const char* functionName) {
    int error = errno;
    //printk("LogicDroid @ throwErrnoException %d %s\n", error, functionName);
...
static rc_t throwIfMinusOne(JNIEnv* env, const char* name, rc_t rc) {
    if (rc == rc_t(-1)) {
	//printk("LogicDroid @ libcore_io_Posix.cpp (throwIfMinusOne)\n");
...
static jobject Posix_socket(JNIEnv* env, jobject, jint domain, jint type, jint protocol) {
    int fd = throwIfMinusOne(env, "socket throwIfMinusOne", TEMP_FAILURE_RETRY(socket(domain, type, protocol)));
    return fd != -1 ? jniCreateFileDescriptor(env, fd) : NULL;
}
