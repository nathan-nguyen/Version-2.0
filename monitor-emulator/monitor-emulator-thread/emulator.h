...
void sys_LogicDroid_unregisterMonitor(void) {
...
//  printf("relationsDataSize = %d", relationsDataSize);
...
History *tNext, *tPrev;
int tRecordIdx, tFreeVarNo, tPolicyNo, tCurrIdx;
const MAX_THREAD = 8;
int tRangeCal[MAX_THREAD][2];

void *threadCal(void *vargp) {
  int index = (int *)vargp;
  int i, j, subIdx, firstCalIdx;

//  printf("\n %d - %d", tRangeCal[index][0], tRangeCal[index][1]);
  for (i = tRangeCal[index][0]; i < tRangeCal[index][1]; i++)
    tNext->propositions[tPolicyNo][tCurrIdx][freeVarCal(tFreeVarNo, i, dependencyTable[tRecordIdx].varIdx[0])] = tNext->propositions[tPolicyNo][dependencyTable[tRecordIdx].fixIdx[2]][freeVarCal(tFreeVarNo, i, dependencyTable[tRecordIdx].varIdx[1])];

  for (i = 2; i < dependencyTable[tRecordIdx].varIdxLength; i++){
    subIdx = dependencyTable[tRecordIdx].fixIdx[i+1];
    for (j = tRangeCal[index][0]; j < tRangeCal[index][1]; j++){
      firstCalIdx = freeVarCal(tFreeVarNo, j, dependencyTable[tRecordIdx].varIdx[0]);
      tNext->propositions[tPolicyNo][tCurrIdx][firstCalIdx] = tNext->propositions[tPolicyNo][tCurrIdx][firstCalIdx] && tNext->propositions[tPolicyNo][subIdx][freeVarCal(tFreeVarNo, j, dependencyTable[tRecordIdx].varIdx[i])];
    }
  }
}
...
int and_function(History *next, History *prev, int recordIdx){
  tNext = next;
  tPrev = prev;
  tRecordIdx = recordIdx;
  tFreeVarNo = dependencyTable[recordIdx].freeVarNo;
  tPolicyNo = dependencyTable[recordIdx].fixIdx[0];
  tCurrIdx  = dependencyTable[recordIdx].fixIdx[1];
  int mainLoopsNo = power(tFreeVarNo);

//  printf ("\nmainLoopsNo: %d",mainLoopsNo);

  if (tFreeVarNo < 3) {
    tRangeCal[0][0] = 0;
    tRangeCal[0][1] = mainLoopsNo;
    threadCal(0);
    return 0;
  }
  ...
  for (int i = 0; i < MAX_THREAD; ++i){
    tRangeCal[i][0] = mainLoopsNo / MAX_THREAD * i;
    tRangeCal[i][1] = (i == MAX_THREAD - 1) ? mainLoopsNo : mainLoopsNo / MAX_THREAD * (i + 1);

    pthread_create(&thread_id[i], NULL, threadCal, i);
  }

  for (int i = 0; i < MAX_THREAD; ++i) pthread_join(thread_id[i], NULL);
...
int freeVarCal(int freeVarNo, int loopNo, VariableIndex varIdx){
  ...
  for (i = 0; i < varIdx.varLength; i++) {
    if (varIdx.var[i].type) value = value * app_num + kfreeVarCurrValue[varIdx.var[i].value];
    else value = value * app_num + varIdx.var[i].value;
  }
...
int power(int number){
  ...
  for (i = 0; i < number; i++) result *= app_num;
